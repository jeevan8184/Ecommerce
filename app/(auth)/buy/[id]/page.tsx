...
import { getProductByColor } from '@/lib/database/actions/product.actions';
import { formatCurrency } from '@/lib/utils';
import { usePathname, useSearchParams } from 'next/navigation'
import React, { Suspense, useContext, useEffect, useState } from 'react'
import { GiShoppingBag } from 'react-icons/gi';

const BuyPageSuspense = ({id}:{id:string}) => {
    ...
    useEffect(()=> {
        const newFunc=async()=>{
            if(id && color && quantity) {

                try {
                    const data=await getProductByColor({
                        productId:id,
                        color,
                        path:pathname,
...
const BuyPage=({params:{id}}:{params:{id:string}})=>{
    return (
        <Suspense fallback={<Loader />}>
            <BuyPageSuspense  id={id} />
        </Suspense>
    )
}
